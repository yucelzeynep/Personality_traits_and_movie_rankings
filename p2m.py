...
#    weights0 = random.rand(5)
    ...
    for idx_person_test in idx_all:
        ...
        delta_personalities_pairwise = tools.get_delta_personalities_pairwise(personalities,weights_opt[idx_person_test])

        idx_person_closest = np.nanargmin(delta_personalities_pairwise[idx_person_test])

        movie_rankings_estimated = \
        movie_rankings[idx_person_closest]

        delta_movie_rankings_estimated = tools.get_delta_movie_ranking_RBSC(
            movie_rankings_estimated,movie_rankings_test
        )
        
        deltas_recommended_by_graph_model[idx_person_test] = delta_movie_rankings_estimated
    ...
    for idx_person_test in idx_all:
        ...
        for i in range(N_RANDOM_ESTIMATIONS):
            random_estimation = []
            while len(random_estimation) < 10:
                n = random.randint(1,10)
                if not n in random_estimation:
                    random_estimation.append(n)


            delta_random_estimation = tools.get_delta_movie_ranking_RBSC(\
                movie_rankings_test,random_estimation)
